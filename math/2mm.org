...
The operator is self-adjoint and hermitian given that any two eigenfunctions $y_i,y_j$ satisfy,

\[
[y_i^* p y_j']_{x=a}^{x=b} = 0
\]
...
\[
y(x) = \int_a^b \left\{ \sum_{n=0}^\infty \frac{1}{\lambda_n}\hat{y}_n(x)\hat{y}^*_n(z)\right\} f(z)\dd{z}
\]
